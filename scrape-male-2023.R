...
# Old data
old_df <- read_csv("data/male-clean-combined-2022.csv")
...
# Save URLS
url_2023 <- "https://interactive.guim.co.uk/docsdata/1J14FwwSaUlGs4M90Jj526ZB7uraCt_VMOgBIJVTKpNI.json"

# Save data to data frames
df_2023 <- jsonlite::fromJSON(url_2023) %>% as.data.frame()

# Write to CSV
write.csv(df_2022, "data/male-raw-2023.csv", row.names=FALSE)
...
colnames(df_2023_mod) <- new_colnames

df_combined <- df_2023_mod %>% 
...
write.csv(df_combined, "data/male-clean-combined-2023.csv", row.names=FALSE)
